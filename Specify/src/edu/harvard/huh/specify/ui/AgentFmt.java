...
/**
 * Formatter class created for BugID 586.
 * 
 * @author mmk
 *
 * Jun 20, 2013
 *
 */
public class AgentFmt implements DataObjDataFieldFormatIFace, Cloneable
{   
    ...
    public String format(final Object dataValue)
    {
        ...
        try
        {
            ...
            if (lastName != null) {
            	...
            	if (firstName != null) name = lastName + ", " + firstName;
            	else name = lastName;
            }
            else {
            	String variant = agent.getVariantName(AgentVariant.FULLNAME);
            	if (variant == null) {
            		Set<AgentVariant> variants = agent.getVariants();
            		if (variants.size() > 0) variant = variants.iterator().next().getName();
            	}
            	name = variant;
            }
            if (name == null) name = emptyName;
            agentName.append(name);
            
            // Acronym
            String acronym = null;
            Byte agentType = agent.getAgentType();
            if (agentType != null && agentType.equals(Agent.ORG)) {
            	String abbrev = agent.getAbbreviation();
            	if (abbrev != null) acronym = " - " + abbrev; 
            }
            
            Agent parentOrg = agent.getOrganization();
            if (parentOrg != null) {
            	String abbrev = parentOrg.getAbbreviation();
            	if (abbrev != null) acronym = " (" + abbrev + ")";
            }
            if (acronym != null) agentName.append(acronym);
            
            // Dates
            String dates = null;
            
            Calendar start = agent.getDateOfBirth();
            Calendar end = agent.getDateOfDeath();
            if (start != null || end != null) {
            	String startYear = null;
            	if (start != null) startYear = String.valueOf(start.get(Calendar.YEAR));
            	
            	String endYear = null;
            	if (end != null) endYear = String.valueOf(end.get(Calendar.YEAR));
            ...
            if (dates != null) agentName.append(dates);
            
        } catch (Exception ex)
        {
            edu.ku.brc.af.core.UsageTracker.incrHandledUsageCount();
            edu.ku.brc.exceptions.ExceptionTracker.getInstance().capture(AgentFmt.class, ex);
            errMsg = ex.toString();
            ex.printStackTrace();
    ...
    {
        AgentFmt codof = (AgentFmt) super.clone();
