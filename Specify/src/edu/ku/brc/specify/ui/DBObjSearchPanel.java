    ...
    protected void doStartQuery(final JComponent comp)
    {
        ...
        } else
        {
            ...
            for (ERTICaptionInfo captionInfo : esTableInfo.getVisibleCaptionInfo())
            {
				...
				// added by HUH: added table abbreviation to WHERE clauses for database object search dialog queries
                String abbrev = "";
                        ...
                        if (qafd.isUserInputNotInjectable(valStr))
                        {
                            if (ESTermParser.getInstance().parse(valStr.toLowerCase(), true))
                            {
                                if (StringUtils.isNotEmpty(valStr))
                                {
                                    ...
                                    if (cnt > 0)
                                    {
                                        strBuf.append(" AND "); // modified by HUH: changed "OR" clauses to "AND" for database object search dialog queries
                                    }
                                    ...
                                    if (clause == null)
                                    {
                                    	// modified by HUH: added table abbreviation to WHERE clauses for database object search dialog queries
                                    	// and use fieldInfo to obtain field name for composing sql query rather than caption@col value
            ...
            try
            {
                ...
                for (Integer id : idList)
                {
                    ...
                    } else
                    {
                        ...
                        // added by HUH: de-duping for DBObjSearchPanel.hasResults()
