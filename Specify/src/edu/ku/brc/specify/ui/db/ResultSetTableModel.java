...
public class ResultSetTableModel extends AbstractTableModel implements SQLExecutionListener, CustomQueryListener
{
    ...
    /* This gets called for the "related" express searches defined in search.config.xml // added by HUH
    ...
    public synchronized void exectionDone(final SQLExecutionProcessor process, final ResultSet resultSet)
    {
		...
		// modified by HUH: major overhaul to apply pick list formatting, handle multiple composite/aggregator captions as defined in search.config.xml
        try
        {
            if (resultSet.next())
            {
                // HUH: This is done once for the result set: add col labels to vector colNames,
                ...
                for (ERTICaptionInfo caption : captions)
                {
                     ...
                     // HUH: why aren't we letting dates be dates?
                     ...
                     // HUH: tell the caption what class its data is
                     caption.setColClass(cls);
                     
                     // HUH: we found an aggregator (this doesn't seem correct to me but it's all we've got)
                ...
                Vector<Object> previousRow = null;  // HUH: the previous displayRow
                ...
                do 
                {
                    ...
                    // HUH: If we are aggregating, only create new display rows for new id values.  Otherwise,
                    ...
                    // HUH: Iterate over the captions.  For each caption there will be a field in the display row.
                    ...
                    // HUH: Captions without aggregators will overwrite their display fields on each update.
                    ...
                    // HUH: displayIndex is where we are in the display values row; it takes into account the 
                    // compression of the values list caused by composite and aggregate captions
                    int displayIndex = 0;

                    for (ERTICaptionInfo caption :  captions)
                    {
                        // HUH: this is the index of the caption in the list of captions we're iterating over
                        int captionIndex  = caption.getPosIndex();

                        // HUH: the result set metadata has an id field at index 0 without a corresponding caption,
                        ...
                        if (isComposite && !isAggregate) // HUH: aggregate captions may have multiple columns too, but they do their own thing
                        {
                        ...
                        else if (isAggregate) // KU: Doing an Aggregation
                        {
                            Object dataObj = null;
                            
                            Object resultSetObj = resultSet.getObject(metadataIndex);
                            
                            if (resultSetObj != null) // HUH: don't bother creating an aggregation object if the result set obj is null
                            {
                            ...
                            if (displayRow.size() <= displayIndex)  // HUH: this is a new row; add a new value and update previous row
                            {
                                ...
                                if (previousRow != null)
                                {
                                    ...
                                    if (previousValue instanceof Vector) // HUH: otherwise we've already updated it and it's a String
                                    {
                            ...
                            else // HUH: this is an old row; update it
                            {
                                if (dataObj != null) ((Vector) displayRow.get(displayIndex)).add(dataObj);
                            }
                        }
                        else // HUH: neither a composite nor an aggregate caption
                        {
                ...
                // KU: We were always setting the rolled up data when the ID changed
                ...
                if (hasAggregatorCaption)
                {
                    ...
                    for (ERTICaptionInfo caption : captions)
                    {
                        ...
                        if (isAggregate)
                        {
                            ...
                            if (value instanceof Vector) // HUH: otherwise we've already updated it and it's a String
                            {
    ...
    /* (non-Javadoc) This gets called for the express searches not defined in search.config.xml // added by HUH
                ...
                for (Object rowObj : list)
                {
                    ...
                    if (rowObj != null && rowObj.getClass().isArray())
                    {
                        ...
                        for (int col=0;col<rowCols.length;col++)
                        {
                            ...
                            // added by HUH: add formatting to display fields in ResultSetTableModel.exectionDone(CustomQueryIFace)
                                       ...
                                       capInfo.getColInfoList().size() > 0)
                            {
                                ...
                                row.add(uiFieldFormatter.formatToUI(val)); // modified by HUH: add formatting to display fields in ResultSetTableModel.exectionDone(CustomQueryIFace)
