        ...
        if (file.exists())
        {
            try
            {
                ...
                for (Object levelObj : root.selectNodes("/tree/treedef/level"))
                {
                    ...
                    String  abbrev       = getAttr(level, "abbrev", null); // added by HUH
                    ...
                    if (rank > -1)
                    {
                        ...
                        treeDefList.add(new TreeDefRow(name, abbrev, title, rank, required, enforced, required && isInFullName, required || rank == 0, sep));
                    }
                }
