            ...
            if (dataObj instanceof Accession)
            {
                ...
                if (accession.getAccessionId() != null)
                {
                    ...
                    try
                    {
                        ...
                        ResultSet rs   = stmt.executeQuery("select count(*) from accessionpreparation where AccessionID = "+accession.getAccessionId()); // modified by HUH: moved relation from collection object to preparation
                        if (rs.next())
                        {
