...
public class DataEntryTask extends BaseTask
{
        ...
        if (subPane == null)
        {
            ...
            if (view != null)
            {
                ...
                else
                {
                    ...
                    mode = "edit"; // added by HUH: make forms default edit view when opened from express search results
                }
                ...
                if (tableInfo != null)
                {
                    if (!tableInfo.isHidden())
                    {
                        ...
                        if (doRegister)
                        {
                            ...
                            // added by HUH: get tooltips and titles for views from objtitle attribute on view element
                            String toolTip = view.getObjTitle();
                            if (toolTip == null) toolTip = dev.getView();
                            
                            // modified by HUH: get tooltips and titles for views from objtitle attribute on view element
    ...
    protected boolean processRecordSetCommand(final CommandAction cmdAction, final List<DataEntryView> list)
    {
        if (ContextMgr.getCurrentContext() == this && cmdAction.getSrcObj() instanceof RecordSetIFace)
        {
            ...
            for (DataEntryView dev : list)
            {
            	// modified by HUH: added null pointer checks
                if (dev != null &&
