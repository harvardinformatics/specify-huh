...
/**
 * Implements drag and drop Filtered Push publishing.  Using "Trash" as a template.
 *
 * @code_status Alpha
 * 
 * @author mkelly
...
public class FpPublish extends JComponent implements GhostActionable
{
    // Static Data Members
    public static final DataFlavor FP_PUB_FLAVOR = new DataFlavor(FpPublish.class, "FpPublish");
    
    private static final FpPublish instance = new FpPublish();
    ...
    protected ImageIcon              fpPublishIcon;
    protected ImageIcon              fpPublishingIcon;
    ...
    protected Vector<DndFpPublishable>   items           = new Vector<DndFpPublishable>();
    protected boolean                isActive            = false;
    protected boolean                isOver              = false;
    protected String                 title               = getResourceString("FpPublish");
    ...
    protected FpPublish()
    {
        dropFlavors.add(FP_PUB_FLAVOR);
        
        fpPublishIcon     = IconManager.getIcon("FpOff");
        fpPublishingIcon = IconManager.getIcon("FpOn");
        paperIcon     = fpPublishingIcon;
        imgIcon       = fpPublishIcon;
        
        hints         = UIHelper.createTextRenderingHints();
        
        //titleFont     = new Font(Font.SANS_SERIF, Font.PLAIN, 11); // XXX Java 6.0
        titleFont     = new Font(createLabel("").getFont().getFamily(), Font.PLAIN, 11);
        titleColor    = new Color(0,0,0,190);

        createMouseInputAdapter();
        
        // XXX RELEASE - Disabled for stand-alone Workbench
        // TODO: FP MMK adapt this somehow?
        if (false)
        {
    ...
    /**
     * Opens the publishing list for viewing TODO: FP MMK keep this?
     */
    protected void openPubList()
    {
        FpPublishDlg dlg = new FpPublishDlg((Frame)UIRegistry.getTopWindow());
        dlg.setVisible(true);        
    }
     ...
     */
    public static FpPublish getInstance()
    {
        return instance;
    }
     ...
     */
    public Vector<DndFpPublishable> getItems()
    {
        return items;
    }
    ...
    /**
     * Runs the drag animation
     */
    protected void fpPublish()
    {
    ...
    public void doAction(GhostActionable src)
    {
        if (src != null)
        {
            ...
            if (src instanceof DndFpPublishable && dataObj != null)
            {
                int option = JOptionPane.showOptionDialog(UIRegistry.getMostRecentWindow(), 
                        String.format(UIRegistry.getResourceString("RecordSetTask.CONFIRM_FP_PUB"), ((DndFpPublishable) src).getName()),
                        UIRegistry.getResourceString("RecordSetTask.CONFIRM_FP_PUB_TITLE"), 
                        JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null, JOptionPane.NO_OPTION); // I18N

                if (option == JOptionPane.YES_OPTION)
                {
                    if (((DndFpPublishable)src).fpPublishRequest())
                    {
                        items.add((DndFpPublishable)src);
                        imgIcon = fpPublishingIcon;
                        repaint();
