...
public class TextFieldWithQuery extends JPanel implements CustomQueryListener
{
    ...
    protected int                              popupTextCharWidth = 100; // added by HUH: increase width of lookup results pop-up to 100 from 25
    ...
    protected Hashtable<String, Object[]>     duplicatehash            = new Hashtable<String, Object[]>(); // modified by HUH: improve duplicate record detection in results lists
    ...
    public void createUI()
    {
        ...
        String tablePrefix = tableInfo.getAbbrev() + "."; // added by HUH: remove table abbreviation prefixes from mouseover text
        for (String k : keyColumns)
        {
            String title = k;
            if (k != null && k.contains(tablePrefix)) k = k.substring(tablePrefix.length()); // added by HUH: remove table prefixes from mouseover text
    ...
    protected String buildSQL(final String newEntryStr, final boolean isForCount)
    {
        ...
        for (String keyCol : keyColumns)
        {
            ...
            addTblAbbrev(tableInfo.getAbbrev(), keyCol, whereSB); // modified by HUH: improve duplicate detection in results lists
        ...
        if (sql == null)
        {
            ...
            if (QueryAdjusterForDomain.getInstance().isUserInputNotInjectable(newEntryStr))
            {
                ...
                if (isForCount)
                {
                    ...
                    addTblAbbrev(tableInfo.getAbbrev(), tableInfo.getIdFieldName(), sb); // modified by HUH: improve duplicate detection in results lists
                    sb.append(")"); //$NON-NLS-1$
    ...
    private void processResults(final CustomQueryIFace customQuery)
    {
        ...
        } else
        {
            ...
            for (Object obj : dataObjList)
            {
                ...
                String key = makeKey(array); // added by HUH: improve duplicate detection in results lists
                
                if (duplicatehash.get(key) == null)
                {
                    duplicatehash.put(key, array); // modified by HUH: improve duplicate detection in results lists
                    ...
                    } else
                    {
                        try
                        {
                            for (int i=0;i<numColumns;i++)
                            {
                                ...
                                {
                                    val = String.valueOf(((Calendar)val).get(Calendar.YEAR)); // modified by HUH: implement search for Agents by AgentVariants 
                            ...
                            if (fontMetrics.stringWidth(valStr) > dim.width)
                            {
                                ...
                                while (len > popupTextCharWidth) // modified by HUH: increase width of lookup results pop-up to 100 from 25
                                {
	...
	// added by HUH: improve duplicate detection in results lists
