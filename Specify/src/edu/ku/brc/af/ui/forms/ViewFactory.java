...
public class ViewFactory
{
                                                           ...
                                                           final boolean       doAdjustQuery)
    {
        ...
        if (isNotEmpty(cbxName))
        {
            ...
            if (cbx != null)
            {
                // added by HUH: get name for search dialog from viewdef cell@initialize("title") for val combo box from query
                                                            ...
                                                            final MultiView     parent)
    {
        ...
        {
            // modified by HUH: implemented search for Agents by AgentVariants
            DataObjectGettable getter = parent.getView().getAltView(null).getViewDef().getDataGettable();
            
            textFieldInfo = TypeSearchForQueryFactory.getInstance().getTextFieldWithInfo(txtName, cellField.getFormatName(), getter);
                               ...
                               final List<FormRowIFace>        formRows)
    {
        ...
        for (FormRowIFace row : formRows)
        {
            ...
            for (FormCellIFace cell : row.getCells())
            {
                ...
                {
                	// modified by HUH: get childInfo from substring of field name following last "." if "." appears, rather than just the whole field name
