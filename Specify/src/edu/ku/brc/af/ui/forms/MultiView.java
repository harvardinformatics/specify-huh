...
public class MultiView extends JPanel
{
    ...
    private void initializeCardPanel(final int options, final String title)
    {
        if (!isOptionOn(options, DONT_USE_EMBEDDED_SEP))
        {
            ...
            String titleStr = view.getObjTitle() != null ? view.getObjTitle() : view.getName();  // modified by HUH: changed initializeCardPanel(int) to use view.getObjTitle() when available rather than getName(); this label appears at the top of a top-level data entry form
