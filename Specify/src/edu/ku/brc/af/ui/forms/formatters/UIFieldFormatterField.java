...
public class UIFieldFormatterField implements Cloneable
{
    ...
    public static UIFieldFormatterField factory(final String   formattingString) throws UIFieldFormatterParsingException 
    {
    	...
    	Pattern pattern = Pattern.compile("^(A+|X+|a+|N+|\\#+|YEAR|Y{4}|Y{2}|\"[^\"]*\")$"); // modified by HUH: implement 2-char date formatting
    ...
    public String getSample()
    {
		...
		{
			sample = Integer.toString(Calendar.getInstance().get(Calendar.YEAR)).substring(4-size, 4); // modified by HUH: implement 2-char date formatting // modified by HUH: implement 2-char date formatting
		}
    ...
    {
        return ((type == FieldType.year) && ((value.equals("YEAR") || value.startsWith("YY")))); // modified by HUH: implement 2-char date formatting
    }
