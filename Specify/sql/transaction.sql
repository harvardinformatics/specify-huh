select t.id,
       ...
       to_char(select min(d.date_due) from due_date where id=t.id) as original_due_date,
       to_char(select max(d.date_due) from due_date where id=t.id) as current_due_date
