...
<xsl:stylesheet version="1.0"
	...
	<xsl:variable name="nameOfContact">
		<xsl:if test="reportAccession/title != ''">
			<xsl:value-of select="reportAccession/title" />
			.&#x20;
		</xsl:if>
		<xsl:value-of select="reportAccession/nameOfContact" />
		,&#x20;
		<xsl:value-of select="reportAccession/jobTitle" />
	</xsl:variable>
	<xsl:variable name="institution">
		<xsl:value-of select="reportAccession/institution" />
		<xsl:if test="reportAccession/acronym != ''">
			- (
			<xsl:value-of select="reportAccession/acronym" />
			)
		</xsl:if>
	</xsl:variable>

	<xsl:variable name="address1" select="reportAccession/address1" />
	<xsl:variable name="address2" select="reportAccession/address2" />
	<xsl:variable name="cityStateZip">
		<xsl:if test="reportAccession/city != ''">
			<xsl:value-of select="reportAccession/city" />
			,
		</xsl:if>
		<xsl:if test="reportAccession/state != ''">
			<xsl:value-of select="reportAccession/state" />
			,
		</xsl:if>
		<xsl:value-of select="reportAccession/zip" />
	</xsl:variable>

	<xsl:variable name="country" select="reportAccession/country" />
	<xsl:variable name="nameOfShippedBy">
		<xsl:value-of select="reportAccession/nameOfShippedBy" />
	</xsl:variable>

	<xsl:variable name="accessionNumber" select="reportAccession/accessionNumber" />
	<xsl:variable name="dateSent" select="reportAccession/dateSent" />
	<xsl:variable name="dateDue" select="reportAccession/dateDue" />
	<xsl:variable name="forUseBy" select="reportAccession/forUseBy" />
	<xsl:variable name="numberOfPackages" select="reportAccession/numberOfPackages" />

	<xsl:variable name="generalCollections" select="reportAccession/generalCollectionCount" />
	<xsl:variable name="nonSpecimens" select="reportAccession/nonSpecimenCount" />
	<xsl:variable name="barcodedSpecimens" select="reportAccession/barcodedSpecimenCount" />
	<xsl:variable name="unbarcodedTotalCount" select="reportAccession/unbarcodedTotalCount" />
	<xsl:variable name="total" select="reportAccession/totalCount" />
	<xsl:variable name="preparationCount" select="reportAccession/preparationCount" />

	<xsl:variable name="description" select="reportAccession/description" />
	<xsl:variable name="accessionCondition" select="reportAccession/accessionCondition" />
	<xsl:variable name="fragmentCount" select="reportAccession/fragmentCount" />
			...
			<fo:page-sequence master-reference="AccessionReport"
				...
				<fo:flow flow-name="xsl-region-body" font="12pt Times New Roman">
									...
									<fo:block>
										<fo:external-graphic content-width="36pt">
											<xsl:attribute name="src">
												<xsl:value-of select="reportAccession/reportsDir" />/logo.png
											</xsl:attribute>
										</fo:external-graphic>
									</fo:block>
								</fo:table-cell>
					...
					<fo:block text-align="center" space-before="12pt"
						space-after="12pt" font-weight="bold" text-decoration="underline">ACCESSION
						SHIPPING
						NOTICE</fo:block>
					...
					<fo:block border-bottom-style="solid" border-bottom-width="2pt"
						...
						<fo:table text-align="left">
							...
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell>
										<fo:block>HUH Accession number:</fo:block>
								    </fo:table-cell>
								    <fo:table-cell>
										<fo:block>
					...
					<xsl:if test="count(reportAccession/unbarcodedSpecimen) &gt; 0">
					...
					<fo:table border-style="solid">
						...
						<fo:table-body>
							<xsl:for-each select="reportAccession/unbarcodedSpecimen">
								<fo:table-row border-style="solid">
									<fo:table-cell padding-right="2mm">
										<fo:block>
					...
					<xsl:if test="count(reportAccession/barcodedSpecimen) &gt; 0">
					...
					<fo:table border-style="solid">
						...
						<fo:table-body>
							<xsl:for-each select="reportAccession/barcodedSpecimen">
								<xsl:choose>
					...
					<xsl:if test="accessionCondition != ''">
					<fo:block space-before="24pt" space-after="12pt">ACCESSION DESCRIPTION
						(<xsl:value-of select="$accessionNumber" />)</fo:block>
					</xsl:if>
						<!--
					...
					<fo:block space-before="12pt">Grand total: ## specimens,
						including
						## types</fo:block> -->
						<fo:block linefeed-treatment="preserve" white-space-collapse="false" white-space-treatment="preserve"><xsl:value-of select="$accessionCondition" /></fo:block>
				</fo:flow>
			</fo:page-sequence>
		</fo:root>
	</xsl:template>
