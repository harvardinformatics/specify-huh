...
<xsl:stylesheet version="1.0"
    ...
    <xsl:variable name="institution">
    	<xsl:value-of select="reportLoan/institution" /> - (<xsl:value-of select="reportLoan/acronym" />)
    </xsl:variable>

	<xsl:variable name="address1"
		select="reportLoan/address1" />
	<xsl:variable name="address2"
		select="reportLoan/address2" />
			...
			<fo:page-sequence master-reference="LoanReport"
				...
				<fo:static-content flow-name="xsl-region-after">
					<fo:block text-align="center">
						...
						Herbarium (GH) - Economic Herbarium of
						Oakes Ames (ECON) &amp;
						Orchid Herbarium of Oakes Ames (AMES) and the
				...
				<fo:flow flow-name="xsl-region-body">
					<fo:block-container>
						...
						<fo:block>22 Divinity Avenue, Cambridge, Massachusetts 02138, USA
						</fo:block>
						<fo:block>
							<fo:inline padding-right="10mm">Tel. 617-495-2365</fo:inline>
					...
					<fo:block><xsl:value-of select="$institution" /></fo:block>
					<fo:block>
					...
					<fo:block space-before="12pt" space-after="36pt">We are sending
						you
						a loan of herbarium specimens as indicated below. Upon arrival of
						this shipment, please verify its contents and acknowledge by
						signing
						one sheet and returning it to the Director at the above address. If
					...
					<fo:block>
						...
						<!--
							(include # of sheets, and # of barcodes if necessary- e.g. 15
							barcodes on 12 sheets) List Geographic Region(s) for vascular
							collections Permission to sample for DNA granted. Permission to
							sample material for chemical analysis granted.
						-->
					</fo:block>
					<fo:block-container space-before="50mm"
						space-after="10mm">
					...
					<fo:table border-style="solid">
						...
						<fo:table-body>
							<xsl:for-each
								select="reportLoan/unbarcodedSpecimen">
								<fo:table-row>
									<fo:table-cell>
										<fo:block>
					...
					<fo:block space-before="12pt" space-after="12pt">Total unbarcoded
						sheet count: <xsl:value-of select="$generalCollections" /></fo:block>
					...
					<fo:table border-style="solid">
						...
						<fo:table-body>
							<xsl:for-each
								select="reportLoan/barcodedSpecimen/barcodedItem">
								<fo:table-row>
									<fo:table-cell>
										<fo:block>
									...
									<fo:table-cell>
										<fo:block><xsl:value-of select="collectorName" />, <xsl:value-of select="collectorNumber" /></fo:block>
									</fo:table-cell>
									<fo:table-cell>
										<fo:block><xsl:value-of select="region" /></fo:block>
									</fo:table-cell>
								</fo:table-row>
							</xsl:for-each>
						</fo:table-body>
					</fo:table>
					<fo:block space-before="12pt" space-after="12pt">Total barcoded
						items: <xsl:value-of select="$barcodedSpecimens" /></fo:block>
					<fo:block space-before="60pt" space-after="12pt">LOAN INVENTORY
